...
nChains = 4
if(is.null(nParallel)) nParallel = 4
Lst = 1
  ...
  if(file.exists(filename.out)){
    ...
    if(file.exists(filename.in)){
      ...
      for(mi in 1:nm){
        ...
        preds = computePredictedValues(m,partition=partition, partition.sp = 1:m$ns, mcmcStep = 100, nParallel = nParallel, nChains = nChains, expected = TRUE, verbose = 50*thin_list)
