...
XData = data %>% dplyr::select(bio1, bio12, bio4, bio15, roadLength, Regolith, forestCover, foliageCover)
...
rL.spatial = setPriors(rL.spatial, nfMin=2, nfMax=4) #limit the model to one latent variables for visualization purpose
...
save(models, file = file.path(modelDir, "unfitted_models_spatial.RData"))
##################################################################################################
...
system.time( # takes ~4 hr to run
  m <- sampleMcmc(models$mFULL, thin = thin, samples = samples, transient = transient, adaptNf = rep(ceiling(0.4*samples*thin),1),
